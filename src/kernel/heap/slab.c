...
void init(ptr_t addr_start) {
    ...
    ptr_t va = addr_start;
    // 映射内存
    map(pgd_kernel, va, pa, VMM_PAGE_PRESENT | VMM_PAGE_RW | VMM_PAGE_KERNEL);
...
ptr_t               alloc_page(ptr_t va, size_t page) {
    ...
    printk_test("100\n");
    printk_test("pa100: 0x%X\n", pa);
    printk_test("va100: 0x%X\n", va);
    for (ptr_t va_start = va, pa_start = pa;
        ...
        if (get_mapping(pgd_kernel, va_start, (ptr_t *)NULL) == 0) {
            map(pgd_kernel, va_start, pa_start,
            ...
            printk_test("addr: 0x%X\n", addr);
            // 这时 addr 就是符合要求的地址
            // 全部映射即可
            map(pgd_kernel, addr, pa_start,
                VMM_PAGE_PRESENT | VMM_PAGE_RW | VMM_PAGE_KERNEL);
            return addr;
        }
    }
    bzero((void *)va, VMM_PAGE_SIZE * page);
    printk_test("pa106: 0x%X\n", pa);
    printk_test("va106: 0x%X\n", va);
    return va;
}
...
ptr_t alloc(size_t byte) {
    ...
    printk_test("new_entry-3: 0x%X\n", new_entry);
    list_init(new_entry);
    printk_test("new_entry-2: 0x%X\n", new_entry);
    // 新表项的可用长度为减去头的大小
    list_slab_block(new_entry)->len =
        (ptr_t)(pages * VMM_PAGE_SIZE) - sizeof(list_entry_t);
    printk_test("new_entry-1: 0x%X\n", new_entry);
    list_add_after(entry, new_entry);
    printk_test("777\n");
    // 进行分割
    printk_test("new_entry1: 0x%X\n", new_entry);
